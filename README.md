...
AI Agent 自主写小说——写、审、改，全程接管。覆盖玄幻、仙侠、都市、科幻等多种风格，支持续写、番外、同人、仿写等创作形式。人工审核门控确保你始终掌控全局。仓库内置 `skills/SKILL.md`，可供兼容 Agent 直接读取。

**InkOS TUI 正式发布！** — 直接运行 `inkos` 进入全屏交互仪表盘。对话式创作、自然语言操作书籍、slash 命令补全、主题动效——TUI、Studio、OpenClaw 共享同一套交互内核。

**InkOS Studio 正式发布！** — `inkos studio` 启动本地 Web 工作台。书籍管理、章节审阅编辑、实时写作进度、市场雷达、数据分析、AI 检测、文风分析、题材管理、守护进程控制、真相文件编辑、提示词工作台——CLI 能做的，Studio 全部可视化。
...
### 通过兼容 Agent / OpenClaw 使用 🦞

通过 npm 安装或克隆本项目时，`skills/SKILL.md` 已包含在内，兼容 Agent（Claude Code、OpenClaw 等）可直接在本地读取；如果你后续把这个 fork 单独发布到 ClawHub，再补对应的 `clawhub install` 命令即可。
...
inkos config set-model reviser <model> --provider <provider>
...
项目级提示词覆盖保存在 `inkos.json` 顶层的 `promptOverrides` 字段，支持两种模式：

- `append`：在解析后的 prompt 末尾追加内容
- `replace`：整段替换原 prompt

这套覆盖是项目级的，适合对单个项目做风格、约束、审计标准的定制，而不需要手改源码。
...
- **提示词工作台**：集中查看真实 prompt 源码片段，并把项目级 prompt override 直接写回 `inkos.json`
...
- **模型温度护栏**：kimi-k2.5 等 thinking 模型自动强制 temperature=1；`kimi-for-coding` 这类仅接受 `<=1` 的模型会自动做上限夹制，避免守护重试时把温度抬爆
...
`inkos up` 启动后台循环自动写章。进入全自动模式后，守护进程会先检查历史章节是否都已经处于“通过/approved”状态；若发现 `ready-for-review` / `audit-failed` 等未通过章节，会先自动补审、按审计意见修订，并在审计通过后自动标记为 `approved`，然后才继续推进下一章。默认会为同一章保留 20 次自动重试预算，并在第 21 次连续失败时暂停该书；这组阈值可通过 `daemon.qualityGates` 调整。自动修订也不再永远卡在 `spot-fix`：命中大纲偏离、读者期待落空、世界规则漂移、时间线矛盾、角色动机/关系连续性断裂等结构性问题时，会自动升级到 `rework`；若同一章已连续失败 4 次，或同时出现多个结构性 `critical`，会进一步升级到 `rewrite`。手动编辑 `inkos.json` 时，请把守护配置合并在同一个顶层 `daemon` 对象里；若只填写了部分 `daemon` 字段，未显式给出的 `schedule` 等嵌套项会自动补默认值。运行期若在审计前就抛异常，Studio 守护页和流水日志会直接显示具体错误文本，不再只显示书名。通知推送支持 Telegram、飞书、企业微信、Webhook（HMAC-SHA256 签名 + 事件过滤）。日志写入 `inkos.log`（JSON Lines），`-q` 静默模式。

补充两点：

- 手动 `inkos write next` 现在也和守护模式一样，会先阻断任何未通过的历史章节；只要存在 `ready-for-review` / `audit-failed` / `state-degraded` 等状态，就不会继续往后写。
- `audit-failed` 章节的候选 truth files 不再直接污染正式 `story/*.md`。它们会先写进书目录下的 review staging；只有 `ready-for-review` 会推进正式 truth/snapshot，而手动 `inkos review approve` 某个 `audit-failed` 章节时，会把这份 staged truth 正式提交为 canon。
- 对于 401/403、Anthropic 鉴权缺失、明显的 baseUrl/模型缺失等非重试型运行错误，`inkos up` 现在会立即暂停该书，不再把这类问题伪装成 20 次 `audit failed` 后才停；普通 400 仍会保留重试，并在日志里附带原始错误文本，便于继续判断是字段兼容、上下文长度还是内容审查问题。
...
| `inkos review approve [id] <n>` | 通过单章；若该章是 `audit-failed`，会一并提交 staged truth |
...
| `inkos up / down` | 启动/停止守护进程（`-q` 静默模式，自动写入 `inkos.log`；`up` 会先补审/补改未通过章节并在通过后自动 approve） |
...
如果你想把当前仓库发到自己的 Git 仓库，并做成“像原版 InkOS 一样 `npm i -g` 一行安装”的形式，关键要分两步：

1. Git 负责托管源码
2. npm 负责提供一行安装的 CLI 包

也就是说，只把代码 push 到 GitHub 还不够；真正让别人执行一行安装的是 npm 上的 `@scope/inkos` 包。
...
如果这是你自己的分支版本，发布前建议先把下面 3 个包名和仓库地址改成你的 scope / repo：
...
如果继续沿用当前的 `@jiejingtazhu/*` 名称，你需要拥有对应 npm 包权限，否则无法发布。
...
- 构建整个 workspace
- 校验 `workspace:*` 依赖在发布时可被替换成真实版本
- 生成 3 个 tarball 到 `tmp/release-packages/`

对应产物分别是：

- `@jiejingtazhu/inkos-core`
- `@jiejingtazhu/inkos-studio`
- `@jiejingtazhu/inkos`

### 3. 配置 GitHub Actions 自动发 npm

当前仓库已经自带 [.github/workflows/release.yml](.github/workflows/release.yml)：

- 推送 `v*` tag 时自动触发
- 先跑测试和 smoke test
- 再按顺序发布 `core -> studio -> cli`
- 先发 canary，再校验，再发 latest
...
如果你改成了自己的 npm scope，发布后用户就可以像原版一样一行安装：

```bash
npm i -g @你的scope/inkos
```

### 4. 安装后直接配置使用

安装完成后，用户只需要配置模型即可开始使用：
