...
const strings = {
  ...
  // Prompts
  "prompts.title": { zh: "提示词工作台", en: "Prompt Workbench" },
  "prompts.subtitle": { zh: "集中查看 InkOS 各处真正会发给模型的提示词入口，并通过项目级 override 做追加或替换。", en: "Inspect the real prompt entry points InkOS sends to models, then apply project-level overrides by append or replace." },
  "prompts.save": { zh: "保存覆盖", en: "Save Overrides" },
  "prompts.systemSource": { zh: "System Prompt 源码片段", en: "System Prompt Source" },
  "prompts.userSource": { zh: "User Prompt 源码片段", en: "User Prompt Source" },
  "prompts.noSystemSource": { zh: "当前条目没有单独的 system prompt 片段。", en: "This entry has no standalone system prompt snippet." },
  "prompts.noUserSource": { zh: "当前条目没有单独的 user prompt 片段。", en: "This entry has no standalone user prompt snippet." },
  "prompts.overrideTitle": { zh: "项目级覆盖", en: "Project Override" },
  "prompts.overrideHint": { zh: "`append` 会把内容追加到解析后的 prompt 后面，`replace` 会整段替换掉当前 prompt。覆盖内容保存在 `inkos.json` 的 `promptOverrides` 字段里。", en: "`append` adds text after the resolved prompt, while `replace` swaps the prompt out entirely. Overrides are stored in `promptOverrides` inside `inkos.json`." },
  "prompts.systemOverride": { zh: "System Override", en: "System Override" },
  "prompts.userOverride": { zh: "User Override", en: "User Override" },
  "prompts.reset": { zh: "恢复默认", en: "Reset" },
  "prompts.overridden": { zh: "已覆盖", en: "Overridden" },
  "prompts.placeholder": { zh: "append 模式会把文本追加到解析后的 prompt 后面；replace 模式会用这里的文本整段替换 prompt。", en: "In append mode, this text is added after the resolved prompt. In replace mode, this text fully replaces the resolved prompt." },
  ...
  "genre.powerScaling": { zh: "战力体系", en: "Power Scaling" },
  "genre.eraResearch": { zh: "时代考据", en: "Era Research" },
