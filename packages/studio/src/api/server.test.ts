...
const readPromptCatalogWithSnippetsMock = vi.fn();
...
vi.mock("@jiejingtazhu/inkos-core", () => {
  ...
  return {
    ...
    readPromptCatalogWithSnippets: readPromptCatalogWithSnippetsMock,
...
describe("createStudioServer daemon lifecycle", () => {
  ...
  beforeEach(async () => {
    ...
    readPromptCatalogWithSnippetsMock.mockReset();
    readPromptCatalogWithSnippetsMock.mockResolvedValue([
      {
        id: "writer.creative-draft",
        group: "agents",
        agent: "writer",
        title: "正文写作",
        description: "Writer 主创作 prompt。",
        systemSnippets: [{
          label: "System Prompt Builder",
          file: "src/agents/writer-prompts.ts",
          content: "export function buildWriterSystemPrompt() {}",
        }],
        userSnippets: [{
          label: "User Prompt Builder",
          file: "src/agents/writer.ts",
          content: "private buildUserPrompt() {}",
        }],
      },
    ]);
  ...
  it("lists prompt catalog entries together with saved prompt overrides", async () => {
    await writeFile(join(root, "inkos.json"), JSON.stringify({
      ...projectConfig,
      promptOverrides: {
        "writer.creative-draft": {
          system: { mode: "append", text: "加一条新约束" },
        },
      },
    }, null, 2), "utf-8");
    ...
    const response = await app.request("http://localhost/api/project/prompts");
  ...
  it("persists prompt overrides back into inkos.json", async () => {
    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
    ...
    const response = await app.request("http://localhost/api/project/prompts", {
      ...
      body: JSON.stringify({
        overrides: {
          "interaction.chat": {
            system: { mode: "replace", text: "You are now terse." },
            user: { mode: "append", text: "answer in one sentence" },
          },
        },
      }),
    });
    ...
    expect(response.status).toBe(200);
    await expect(readFile(join(root, "inkos.json"), "utf-8")).resolves.toContain("\"promptOverrides\"");
    await expect(readFile(join(root, "inkos.json"), "utf-8")).resolves.toContain("interaction.chat");
    await expect(readFile(join(root, "inkos.json"), "utf-8")).resolves.toContain("You are now terse.");
  });
