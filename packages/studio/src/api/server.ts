...
export function createStudioServer(initialConfig: ProjectConfig, root: string) {
  ...
  app.get("/api/project/prompts", async (c) => {
    const configPath = join(root, "inkos.json");
    const raw = JSON.parse(await readFile(configPath, "utf-8")) as { promptOverrides?: unknown };
    const prompts = await readPromptCatalogWithSnippets();
    return c.json({
      prompts,
      overrides: raw.promptOverrides && typeof raw.promptOverrides === "object"
        ? raw.promptOverrides
        : {},
    });
  });

  app.put("/api/project/prompts", async (c) => {
    const { overrides } = await c.req.json<{ overrides?: unknown }>();
    const configPath = join(root, "inkos.json");
    const raw = JSON.parse(await readFile(configPath, "utf-8")) as Record<string, unknown>;
    raw.promptOverrides = overrides && typeof overrides === "object" ? overrides : {};
    const { writeFile: writeFileFs } = await import("node:fs/promises");
