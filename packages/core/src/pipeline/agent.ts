...
export async function runAgentLoop(
  ...
  const systemPrompt = `你是 InkOS 小说写作 Agent。用户是小说作者，你帮他管理从建书到成稿的全过程。
...
- 不要在没有确认书籍状态的情况下直接调用写作工具`;
  const prompts = await applyPromptOverridePair(config.projectRoot, "pipeline.agent-loop", {
    system: systemPrompt,
    user: instruction,
  });
  ...
  const messages: AgentMessage[] = [
    ...
    { role: "user", content: prompts.user ?? instruction },
  ];
