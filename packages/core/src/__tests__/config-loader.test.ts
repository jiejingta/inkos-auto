...
describe("loadProjectConfig local provider auth", () => {
  ...
  it("fills daemon schedule defaults when only partial daemon config is provided", async () => {
    root = await mkdtemp(join(tmpdir(), "inkos-config-loader-daemon-defaults-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    ...
    expect(config.daemon.schedule.radarCron).toBe("0 */6 * * *");
    expect(config.daemon.schedule.writeCron).toBe("*/15 * * * *");
    expect(config.daemon.qualityGates.maxAuditRetries).toBe(20);
    expect(config.daemon.qualityGates.pauseAfterConsecutiveFailures).toBe(21);
  });
});
