...
export class WriterAgent extends BaseAgent {
  ...
  async writeChapter(input: WriteChapterInput): Promise<WriteChapterOutput> {
    ...
    const creativePrompts = await this.applyPromptOverride("writer.creative-draft", {
      system: creativeSystemPrompt,
      user: creativeUserPrompt,
    });
    ...
    const creativeResponse = await this.chat(
      [
        { role: "system", content: creativePrompts.system ?? creativeSystemPrompt },
        { role: "user", content: creativePrompts.user ?? creativeUserPrompt },
      ],
  ...
  private async settle(params: {
    ...
    const observerPrompts = await this.applyPromptOverride("writer.observe-chapter", {
    ...
    const observerResponse = await this.chat(
      [
        { role: "system", content: observerPrompts.system ?? observerSystem },
        { role: "user", content: observerPrompts.user ?? observerUser },
      ],
    ...
    const settlerPrompts = await this.applyPromptOverride("writer.settle-state", {
      system: settlerSystem,
      user: settlerUser,
    });
    ...
    const response = await this.chat(
      [
        { role: "system", content: settlerPrompts.system ?? settlerSystem },
        { role: "user", content: settlerPrompts.user ?? settlerUser },
      ],
