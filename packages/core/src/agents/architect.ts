...
${finalRequirementsPrompt}`;
    ...
    const prompts = await this.applyPromptOverride("architect.init-book", {
      system: langPrefix + systemPrompt,
      user: userMessage,
    });

    const response = await this.chat([
      { role: "system", content: prompts.system ?? langPrefix + systemPrompt },
      { role: "user", content: prompts.user ?? userMessage },
...
${keyPrinciplesPrompt}`;
    ...
    const prompts = await this.applyPromptOverride("architect.import-foundation", {
      system: systemPrompt,
      user: userMessage,
    });
    ...
    const response = await this.chat([
      ...
      {
        role: "user",
        content: prompts.user ?? userMessage,
      },
...
初始状态卡（基于正典起始点）
    ...
    === SECTION: pending_hooks ===
    初始伏笔池（从正典关键事件和关系中提取）`;
    const userPrompt = `请为标题为"${book.title}"的${fanficMode}模式同人小说生成基础设定。目标${book.targetChapters}章，每章${book.chapterWordCount}字。`;
    const prompts = await this.applyPromptOverride("architect.init-fanfic", {
      system: systemPrompt,
      user: userPrompt,
    });
    ...
    const response = await this.chat([
      ...
      {
        role: "user",
        content: prompts.user ?? userPrompt,
      },
