...
const STATE_POLLUTION_WARNING_PATTERNS = [
  ...
  /power scaling|numerical consistency|resource|ledger|战力|资源|数值|账本/u,
  /pov|information boundary|future knowledge leak|knowledge leak|视角|信息越界|知识泄露/u,
  /relationship|motivation|incentive chain|关系|动机/u,
] as const;

export function formatAuditIssue(issue: Pick<AuditIssue, "severity" | "category" | "description">): string {
  const category = issue.category.trim();
  const description = issue.description.trim();
  if (category.length === 0) {
    return `[${issue.severity}] ${description}`;
  }
  if (description.length === 0) {
    return `[${issue.severity}] ${category}`;
  }
...
function localizeGateMessage(language: PromptLanguage, key: "critical" | "state" | "warning-count" | "repeated"): string {
  if (language === "en") {
    switch (key) {
      case "critical":
        return "contains critical issues";
      case "state":
...
export function evaluateEditorialAuditGate(params: {
  ...
  if (criticalIssues.length > 0) {
    rejectionReasons.push(localizeGateMessage(language, "critical"));
  }
...
## 待审章节内容
${chapterContent}`;
    const prompts = await this.applyPromptOverride("continuity.audit-chapter", {
      system: systemPrompt,
      user: userPrompt,
    });

    const chatMessages = [
      { role: "system" as const, content: prompts.system ?? systemPrompt },
      { role: "user" as const, content: prompts.user ?? userPrompt },
    ];
    ...
    const summary = gateSummary.length > 0
      ? [result.summary.trim(), gateSummary].filter((part) => part.length > 0).join(isEnglish ? " | " : "；")
      : result.summary;
    return {
      ...result,
