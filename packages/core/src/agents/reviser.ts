...
import type { AuditIssue, AuditTruthContext } from "./continuity.js";
...
export class ReviserAgent extends BaseAgent {
  ...
  async reviseChapter(
    ...
    const officialCurrentState = options?.truthContext?.official?.currentState ?? currentState;
    const officialLedger = options?.truthContext?.official?.ledger ?? ledger;
    const officialHooks = options?.truthContext?.official?.hooks ?? hooks;
    const candidateTruth = options?.truthContext?.candidate;
    ...
    const lengthGuardrail = options?.lengthSpec
      ? `\n9. 保持章节字数在目标区间内；只有在修复关键问题确实需要时才允许轻微偏离`
      : "";
...
7. 修改后同步更新状态卡${gp.numericalSystem ? "、账本" : ""}、伏笔池
 8. 同时维护与相邻章节的连续性：前文已落地事实不能回写，若后续章节已存在则不得制造冲突
${lengthGuardrail}
${mode === "spot-fix" ? "\n10. spot-fix 只能输出局部补丁，禁止输出整章改写；TARGET_TEXT 必须能在原文中唯一命中\n11. 如果需要大面积改写，说明无法安全 spot-fix，并让 PATCHES 留空" : ""}
...
${globalWritingRules}`;
    ...
    const stateHeading = candidateTruth
      ? isEnglish
        ? "Official Pre-Chapter State Card (revision starting point)"
        : "官方章前状态卡（修订起点）"
      : isEnglish
        ? "Current State Card"
        : "当前状态卡";
    const ledgerHeading = candidateTruth
      ? isEnglish
        ? "Official Pre-Chapter Resource Ledger"
        : "官方章前资源账本"
      : isEnglish
        ? "Resource Ledger"
        : "资源账本";
    const hooksHeading = candidateTruth
      ? isEnglish
        ? "Official Pre-Chapter Pending Hooks"
        : "官方章前伏笔池"
      : isEnglish
        ? "Pending Hooks"
        : "伏笔池";
    ...
    const hooksBlock = governedMemoryBlocks?.hooksBlock
      ?? `\n## ${hooksHeading}\n${hooksWorkingSet}\n`;
    const candidateTruthBlock = candidateTruth
      ? isEnglish
        ? `\n## Candidate Post-Chapter Truth (revision target, not the starting state)\nUse this block as the settled truth produced by the current draft. Repair the chapter so it can legitimately move from the official pre-chapter truth to this post-chapter truth; do not treat it as facts already true before the chapter starts.\n\n### Candidate Post-Chapter State Card\n${candidateTruth.currentState ?? "(not provided)"}\n${gp.numericalSystem ? `\n### Candidate Post-Chapter Resource Ledger\n${candidateTruth.ledger ?? "(not provided)"}\n` : ""}\n### Candidate Post-Chapter Pending Hooks\n${candidateTruth.hooks ?? "(not provided)"}\n`
        : `\n## 候选章后真相（修订目标，不是章初状态）\n这一块是当前待修正文结算出来的章后真相。请把章节修到能够从“官方章前真相”合理推进到这里；不要把它当成章节开始前已经成立的事实。\n\n### 候选章后状态卡\n${candidateTruth.currentState ?? "(未提供)"}\n${gp.numericalSystem ? `\n### 候选章后资源账本\n${candidateTruth.ledger ?? "(未提供)"}\n` : ""}\n### 候选章后伏笔池\n${candidateTruth.hooks ?? "(未提供)"}\n`
      : "";
    ...
    const chapterTrailBlock = continuityPack.chapterTrail
      ? isEnglish
        ...
        : `\n## 邻近章节轨迹（N-3..N+1）\n${continuityPack.chapterTrail}\n`
      : "";
    const previousChapterBlock = continuityPack.previousChapterFullText
      ? isEnglish
        ? `\n## Previous Chapter Full Text (must keep the transition)\n${continuityPack.previousChapterFullText}\n`
        : `\n## 上一章全文（必须保持承接）\n${continuityPack.previousChapterFullText}\n`
      : "";
    const nextChapterBlock = continuityPack.nextChapterOpening
      ? isEnglish
        ? `\n## Next Chapter Opening (already written; avoid back-writing conflicts)\n${continuityPack.nextChapterOpening}\n`
        : `\n## 下一章开头（若已存在，避免回写冲突）\n${continuityPack.nextChapterOpening}\n`
      : "";
...
${chapterTrailBlock}${previousChapterBlock}${nextChapterBlock}
## ${stateHeading}
${officialCurrentState}
${ledgerBlock}
${hookDebtBlock}${hooksBlock}${candidateTruthBlock}${volumeSummariesBlock}${reducedControlBlock || outlineBlock}${bibleBlock}${matrixBlock}${summariesBlock}${canonBlock}${fanficCanonBlock}${styleGuideBlock}${lengthGuidanceBlock}

## 待修正章节
${chapterContent}`;
    const prompts = await this.applyPromptOverride("reviser.revise-chapter", {
      system: systemPrompt,
      user: userPrompt,
    });
    ...
    const mergedOutput = governedMode
      ? {
          ...output,
          updatedHooks: mergeTableMarkdownByKey(officialHooks, output.updatedHooks, [0]),
        }
      : output;
