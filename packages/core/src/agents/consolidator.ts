...
export class ConsolidatorAgent extends BaseAgent {
  ...
  async consolidate(bookDir: string): Promise<ConsolidationResult> {
    ...
    for (const vol of completedVolumes) {
      ...
      const systemPrompt = "You are a narrative summarizer. Compress chapter-by-chapter summaries into a single coherent paragraph (max 500 words) that captures the key events, character developments, and plot progression of this volume. Preserve specific names, locations, and plot points. Write in the same language as the input.";
      const userPrompt = `Volume: ${vol.name} (Chapters ${vol.startCh}-${vol.endCh})\n\nChapter summaries:\n${header}\n${volSummaryRows}`;
      const prompts = await this.applyPromptOverride("consolidator.volume-summary", {
        system: systemPrompt,
        user: userPrompt,
      });
      ...
      const response = await this.chat([
        {
          role: "system",
          content: prompts.system ?? systemPrompt,
        },
        {
          role: "user",
