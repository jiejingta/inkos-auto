...
export function createInteractionToolsFromDeps(
  ...
  return {
    ...
    developBookDraft: async (input, existingDraft) => {
      ...
      const draftUserPrompt = JSON.stringify({
        currentDraft: existingDraft ?? null,
        latestMessage: input,
      }, null, 2);
      const draftPrompts = await applyPromptOverridePair(
      ...
      const response = await tracedChatCompletion(
        ...
        [
          {
            role: "system",
            content: draftPrompts.system ?? draftSystemPrompt,
          },
          {
            role: "user",
            content: draftPrompts.user ?? draftUserPrompt,
          },
        ],
        { temperature: 0.4 },
        {
    ...
    chat: async (input, options) => {
      ...
      if (instrumentedPipeline.config?.client && instrumentedPipeline.config?.model) {
        try {
          ...
          const chatUserPrompt = `activeBook=${bookLabel}\nautomationMode=${options.automationMode}\nmessage=${input}`;
          const chatPrompts = await applyPromptOverridePair(
            instrumentedPipeline.config.projectRoot ?? "",
            "interaction.chat",
            {
          ...
          response = await tracedChatCompletion(
            ...
            [
              {
                role: "system",
                content: chatPrompts.system ?? chatSystemPrompt,
              },
              {
                role: "user",
                content: chatPrompts.user ?? chatUserPrompt,
              },
            ],
            {
