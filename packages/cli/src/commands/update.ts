...
export const updateCommand = new Command("update")
  ...
  .action(async () => {
    try {
      ...
      const remoteVersion = execSync("npm view @jiejingtazhu/inkos version", {
        encoding: "utf-8",
      }).trim();
      ...
      execSync("npm install -g @jiejingtazhu/inkos@latest", { stdio: "inherit" });
      log(`Updated to ${remoteVersion}.`);
    } catch (e) {
      logError(`Update failed: ${e}`);
      log("You can also update manually: npm install -g @jiejingtazhu/inkos@latest");
      process.exit(1);
    }
  });
