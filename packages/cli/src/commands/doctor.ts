...
export const doctorCommand = new Command("doctor")
  ...
  .action(async (opts: { repairNodeRuntime?: boolean }) => {
    ...
    // 5. Check LLM API key (global + project .env)
    {
      ...
      const { isApiKeyOptionalForEndpoint } = await import("@jiejingtazhu/inkos-core");
    ...
    try {
      const { StateManager } = await import("@jiejingtazhu/inkos-core");
    ...
    try {
      const { createLLMClient, chatCompletion, LLMConfigSchema, isApiKeyOptionalForEndpoint } = await import("@jiejingtazhu/inkos-core");
      const { loadConfig } = await import("../utils.js");
