...
export async function resolveStudioLaunch(root: string): Promise<StudioLaunchSpec | null> {
  ...
  if (sourceEntry) {
    ...
    if (localTsxLoader) {
      return {
        ...
        command: "node",
        args: ["--import", toFileSpecifier(localTsxLoader), sourceEntry, root],
      };
    }
  ...
  const builtEntry = await firstAccessiblePath([
    join(root, "node_modules", "@jiejingtazhu", "inkos-studio", "dist", "api", "index.js"),
    join(root, "node_modules", "@jiejingtazhu", "inkos-studio", "server.cjs"),
    join(cliPackageRoot, "node_modules", "@jiejingtazhu", "inkos-studio", "dist", "api", "index.js"),
    join(cliPackageRoot, "node_modules", "@jiejingtazhu", "inkos-studio", "server.cjs"),
