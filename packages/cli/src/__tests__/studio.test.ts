...
describe("studio command", () => {
  ...
  it("launches monorepo TypeScript sources through node --import with file URLs when tsx loader exists", async () => {
    const tsEntry = join("/project", "packages", "studio", "src", "api", "index.ts");
    ...
    const { studioCommand } = await import("../commands/studio.js");
    await studioCommand.parseAsync(["node", "studio", "--port", "9002"]);
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: "/project",
        stdio: "inherit",
        env: expect.objectContaining({ INKOS_STUDIO_PORT: "9002" }),
      }),
    );
  });
