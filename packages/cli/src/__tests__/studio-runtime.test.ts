...
describe("studio runtime resolution", () => {
  ...
  it("wraps Windows absolute source paths as file URLs for node --import", async () => {
    const windowsRoot = "D:\\repo\\test-project";
    const windowsSourceEntry = "D:\\repo\\packages\\studio\\src\\api\\index.ts";
    const windowsTsxLoader = "D:\\repo\\packages\\studio\\node_modules\\tsx\\dist\\loader.mjs";
    ...
    expect(launch).toEqual({
      ...
      args: [
        "--import",
        pathToFileURL(windowsTsxLoader).href,
        windowsSourceEntry,
        windowsRoot,
      ],
    });
